...
TupleContainer::TupleContainer(float weight):
Tuple(weight),
tuples_()
{
}
...
float TupleContainer::query_tuple_value(const BitBoard& game_board) const
{
...
void TupleContainer::update_tuple_value(const BitBoard& game_board, const float& value)
{
	for(vector<Tuple*>::iterator it = tuples_.begin(); it != tuples_.end(); ++it)
